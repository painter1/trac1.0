...
class NotifyEmail(Notify):
    ...
    def send(self, torcpts, ccrcpts, mime_headers={}):
        ...
        if not mime_headers.has_key('X-Mailer'):
            headers['X-Mailer'] = 'Trac %s, by Edgewall Software' % __version__
        if not mime_headers.has_key('X-Trac-Version'):
            headers['X-Trac-Version'] =  __version__
        if not mime_headers.has_key('X-Trac-Project'):
            headers['X-Trac-Project'] =  self.env.project_name
        if not mime_headers.has_key('X-URL'):
            headers['X-URL'] = self.env.project_url
        headers['Precedence'] = 'bulk'
        headers['Auto-Submitted'] = 'auto-generated'
        if not mime_headers.has_key('Subject'):
            headers['Subject'] = self.subject
        if not mime_headers.has_key('From'):
            headers['From'] = (self.from_name, self.from_email) if self.from_name \
                else self.from_email
        if not mime_headers.has_key('Reply-To'):
        ...
        # PCMDI Customication: mime_heades has already been copied into headers:
